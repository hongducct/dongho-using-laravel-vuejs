...
const handleUpdateEmail = async (newEmail) => {
  try {
    ...
    const response = await axios.put(`${apiUrl}/user/${user.value.id}`, {
      email: newEmail,
    });
...
const changeAvatar = async () => {
  try {
    ...
    if (file) {
      ...
      // Cập nhật thuộc tính avatar của user.value
      user.value.avatar = updatedUserResponse.data.data.avatar;
    }
  } catch (error) {
